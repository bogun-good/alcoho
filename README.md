...
<script>
    ...
    function restartGame() {
        ...
        score = 0; // 점수 초기화
        mcCorrectCount = 0; // 객관식 정답 수 초기화
        mcIncorrectCount = 0; // 객관식 오답 수 초기화
        userDescriptiveAnswers = []; // 주관식 답변 초기화
        optionsDisabled = false; // 보기 클릭 활성화
        finalResultDiv.style.display = "none"; // 결과 화면 숨김
        questionBox.style.display = "block"; // 문제 상자 표시
        optionsContainer.style.display = "flex"; // 보기 컨테이너 표시
        car.style.display = "flex"; // 자동차 표시
        scoreDisplay.style.display = "block"; // 점수 표시
        feedback.style.display = "block"; // 피드백 표시
        scoreDisplay.innerText = "점수: 0"; // 점수 초기화 표시
